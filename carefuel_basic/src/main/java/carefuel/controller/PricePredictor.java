...
public class PricePredictor {
	...
	/**
	 * This function interpolates the data given by 'datePriceList' at every hour linearly.
	 * @param datePriceList should contain a list of ordered dates and prices from the beginning
	 * to the last entry, where the last entry has to be slightly AFTER 'maxDate'
	 * @param maxDate is the last date that can be used for the prediction
	 *
	 * @return Returns a two dimensional array of interpolated data. The first index corresponds
	 * to the month and second to the hour of the month, resulting in the shape [?][nHoursPerMonth].
	 * The data is in reversed order and begins at maxDate.
	 */
	private float[][] interpolatePrices(List<Pair<Date, Integer>> datePriceList, Date maxDate){
		...
		long diff_hours = (maxDate.getTime() - datePriceList.get(0).getLeft().getTime()) /
				(3600 * 1000); //number of hours contained
	...
	/**
	 * Convenience function. Calls predictNextMonth and calculates the price at predictionDate.
	 * See predictNextMonth for more information.
	...
	public int predictPrice(String maxDateString, String predictionDateString, int gasStationID) throws Exception
	{
		...
		// Get the number"2017-08-21 23:03:06+02" of hours from maxDate and predictionDate
	...
	/**
	 * This function uses the neural network in order to predict all prices of a given
	 * gas station using for the next month since 'maxDate'. It uses all previous data
	 * before 'maxDate'. Note that 'maxDate' has to be BEFORE the last entry in the
	 * corresponding data set of the gas station.
	 * @param maxDateString beginning of the month at which to predict
	...
	public float[] predictNextMonth(String maxDateString, int gasStationID) throws Exception
	{
		...
		/*
		 * Interpolate the data from the first to the entry at lastEntryIndex at every hour
		 * The array interpolatedPrices is of the form [?][nHoursPerMonth], where ? is the
		 * number of "whole" months, i.e. every month that still has 31 days á 24 hours of
		 * entries. Note that the array is in reversed form, meaning it goes back in time.
		 * Therefore, interpolatedPrices[0][0] contains the last price of the month before
		 * the month that needs to be predicted, interpolatedPrices[0][1] the price before
		 * that and so on.
		 */
		 ...
		 * dimension of the input tensor (even though not all data from the first
		 * months may be accessible). Therefore, fill in the according number
		 * of zero arrays
		 */
		...
		//Create the first input tensor of shape (1, 50, 744) containing the previous months
